...
var cue_names  = jsPsych.randomization.repeat(["Quizzing", "Re-reading"], 1);
jsPsych.data.addProperties({
  counterbalance_left: cue_names[0],
  counterbalance_right: cue_names[1]
});
...
const efficacy_instructions = {
  ...
  pages: [
    ...
    <p>On the next screen you will see a slider labelled from &ldquo;${cue_names[0]} more effective&rdquo; (left) to &ldquo;${cue_names[1]} more effective&rdquo; (right).</p>
    `,
    ...
    <p style="max-width:60ch; margin:0 auto; text-align:left;">
      &bull; If you feel <strong>${cue_names[0]}</strong> helped you remember the pairs <em>much more</em> than ${cue_names[1]}, drag the slider far to the left.<br><br>
      &bull; If you feel <strong>${cue_names[1]}</strong> helped <em>much more</em> than ${cue_names[0]}, drag it far to the right.<br><br>
...
function initStimuli(cond){
  let pres;
  if (cond === 0){
    jsPsych.data.addProperties({Cue_Density_Condition: "Frequent Re-reading"});
    pres = jsPsych.randomization.repeat(["Read","Quiz"], [28,12]);
  } else if (cond === 1){
    jsPsych.data.addProperties({Cue_Density_Condition: "Frequent Quizzing"});
    pres = jsPsych.randomization.repeat(["Read","Quiz"], [12,28]);
  } else {
    jsPsych.data.addProperties({Cue_Density_Condition: "Balanced"});
              ...
              // Feedback trial
                  {
                    ...
                    stimulus: function () {
                      var stim = jsPsych.evaluateTimelineVariable('cue') + ' - ' + jsPsych.evaluateTimelineVariable('target');
                      return `<p style="color: gray;">Feedback</p><p style="font-size: 28pt;"><strong>${stim}</strong></p>`;
                    },
                    choices: "NO_KEYS",
                    trial_duration: study_time,
                    on_finish: function(data){
                      data.trial_type = "Quiz Feedback";
                    }
                  }
...
*/
    ...
    const sliderHTML = `
      <div class="custom-slider-container">
        ...
        <div class="custom-slider-labels"><span>${cue_names[0]} more effective</span><span>No Difference</span><span>${cue_names[1]} more effective</span></div>
      </div>
      <p class="prompt-below">Which method was more effective?</p>`;
